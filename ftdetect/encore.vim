...
au BufEnter,BufRead,BufNewFile *.enc call g:encore_map()
au BufLeave  *.enc call g:encore_unmap()

let s:encore_is_mapped=0

function! g:encore_map()
  if !s:encore_is_mapped
    ...
    map <leader>ed idef <++>(<++>): <++> {<CR><TAB><++><CR><ESC>2"_dhi}<ESC><ESC>2k/<++><CR>4cl
